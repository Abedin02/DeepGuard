  ...
  const fetchPosts = async () => {
    try {
      ...
      if (data.status === 'ok') {
        ...
        console.error('Error fetching NYT news:', data.error || data.message)
      }
    } catch (error) {
      ...
      >
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          ...
          <h2 style={{ fontSize: '2rem', fontWeight: '900', color: theme.text, letterSpacing: '2px', textTransform: 'uppercase', margin: 0, transition: 'color 0.3s ease' }}>
            <DiaTextReveal text="The Deepfake Threat Is " />
            <span style={{ color: theme.primary, transition: 'color 0.3s ease' }}>
              <DiaTextReveal text="Real" />
            </span>
          </h2>
        </div>
  ...
  return (
    ...
    >
      ...
      <a
        ...
        style={{ maxWidth: '820px', margin: '0 auto', backgroundColor: theme.cardBg, border: `1px solid ${theme.border}`, borderRadius: '16px', padding: '2.5rem 3rem', display: 'grid', gridTemplateColumns: '1fr 160px', gap: '2.5rem', alignItems: 'center', opacity: fading ? 0 : 1, transform: fading ? `translateX(${dir === 'right' ? '-20px' : '20px'})` : 'translateX(0)', transition: 'opacity 0.3s ease, transform 0.3s ease, border-color 0.3s ease', boxSizing: 'border-box', cursor: 'pointer', minHeight: '180px', overflow: 'hidden', textDecoration: 'none' }}
        onMouseEnter={e => { setIsHovered(true); e.currentTarget.style.borderColor = theme.primary; e.currentTarget.style.opacity = '0.9' }}
        onMouseLeave={e => { setIsHovered(false); e.currentTarget.style.borderColor = theme.border; e.currentTarget.style.opacity = '1' }}
      >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1.25rem', marginTop: '2rem' }}>
        <RippleButton onClick={prev} style={{ background: 'transparent', border: `1px solid ${theme.border}`, color: theme.muted, width: '32px', height: '32px', borderRadius: '4px', cursor: 'pointer', fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'border-color 0.2s, color 0.2s' }} onMouseEnter={e => { e.currentTarget.style.borderColor = theme.primary; e.currentTarget.style.color = theme.primary }} onMouseLeave={e => { e.currentTarget.style.borderColor = theme.border; e.currentTarget.style.color = theme.muted }}>←</RippleButton>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          {posts.map((_, i) => <RippleButton key={i} aria-label={`Show article ${i + 1}`} onClick={() => goTo(i, i > active ? 'right' : 'left')} style={{ width: i === active ? '22px' : '5px', height: '5px', borderRadius: '3px', background: i === active ? theme.primary : theme.border, border: 'none', cursor: 'pointer', padding: 0, transition: 'width 0.3s ease, background 0.3s ease' }} />)}
        </div>
        <RippleButton onClick={next} style={{ background: 'transparent', border: `1px solid ${theme.border}`, color: theme.muted, width: '32px', height: '32px', borderRadius: '4px', cursor: 'pointer', fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'border-color 0.2s, color 0.2s' }} onMouseEnter={e => { e.currentTarget.style.borderColor = theme.primary; e.currentTarget.style.color = theme.primary }} onMouseLeave={e => { e.currentTarget.style.borderColor = theme.border; e.currentTarget.style.color = theme.muted }}>→</RippleButton>
      </div>
    </section>
  )
}
